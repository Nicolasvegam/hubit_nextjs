...
export async function loadServicesSuggestions( uid ) {
    ...
    for (let i = 0; i < cars.length; i++) {
        ...
        let { data: subscription } = await supabase.from('subscriptions').select('name').eq('id', subType.suid).single()
        let suggestions = []
        if ( subType.active ) {
            for (let j = 0; j < benefits.length; j++) {
                let { data: service }  = await supabase.from('services').select('*').eq('id', benefits[j].sid).single()
        ...
        cars[i].model = model
        cars[i].subscription = subscription.name
